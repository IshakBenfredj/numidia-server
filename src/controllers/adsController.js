...
export const createAd = async (req, res) => {
  try {
    ...
    // Upload image to R2
    const imageUrl = await uploadSingleImage(imageBase64, "ads");

    const ad = await Ads.create({
      title: title || "إعلان جديد",
      image: {
        url: imageUrl,           // Now we only store the URL
        // public_id is no longer needed with R2
      },
      clickUrl: clickUrl || null,
      isActive: isActive !== undefined ? isActive : true,
    });
...
export const updateAd = async (req, res) => {
  try {
    ...
    // If new base64 image is provided → upload to R2 and delete old one
    if (imageBase64 && typeof imageBase64 === "string" && imageBase64.startsWith("data:image")) {
      const newImageUrl = await uploadSingleImage(imageBase64, "ads");

      // Delete old image from R2 if it exists
      if (ad.image?.url) {
        await deleteFileByUrl(ad.image.url).catch((err) =>
          console.error("Failed to delete old ad image:", err)
        );
      }
